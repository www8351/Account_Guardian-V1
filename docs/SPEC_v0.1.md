...
MQL5/Include/AccountGuardian/Sweep.mqh             flatten engine, pending deletion, the only trade API reach
MQL5/Include/AccountGuardian/SweepPolicy.mqh       sweep policy, pure helpers, no trade API (ENF-24(b), 2026-09-16)
...
Static-structure rule, checkable without running: the trade API (OrderSend and relatives) is reachable only from Sweep.mqh. Pnl.mqh, Clock.mqh, Log.mqh, State.mqh, Persist.mqh and SweepPolicy.mqh contain no trade calls. The weekly path is inside Pnl.mqh and therefore provably cannot trade. Measured on every build by static rows ENF-S2 (the trade API family confined to Sweep.mqh) and ENF-S3 (the sweep pass reachable from the LOCKED dispatch and the OnTradeTransaction accelerator only), owner ruling ENF-23(a) of 2026-09-16.
...
- Breach: daily PnL <= -(limit_currency), epsilon 0.01 account-currency units errs toward breach. On breach: snapshot, persist, lock, delete all pending orders, flatten all positions. **Enforcement (owner rulings ENF-1 to ENF-29 of 2026-09-16, recorded under the ruling and not as a new amendment):** the breach pass locks and the sweep engine in Sweep.mqh runs from the first LOCKED timer pass after the transition, on every LOCKED pass while anything is open. The Phase 1 interim posture (A6, Q2 FINAL, alert only and stay ACTIVE) and the Phase 2 interim posture (2026-08-18, lock but send no order) were each owner-sanctioned windows and are both closed; the 2026-09-16 ruling supersedes the Phase 2 posture in its no-order clause and in that clause only.
...
- Sweep (behavior of LOCKED): every timer tick while anything is open, one pass over all positions and pendings; per-position backoff; a distinct log line per failure retcode; no unbounded tight retry. As ruled 2026-09-16 (ENF-1 to ENF-22): the trigger is the breach and every LOCKED tick while any position or pending exists (ENF-1(b)), with one extra pass on a `TRADE_TRANSACTION_DEAL_ADD` whose deal is an entry while LOCKED, acceleration only (ENF-2(a), F13); the scope is every position and pending on the account regardless of origin, symbol or magic (ENF-3(a)); the send mode is synchronous, at most ONE send per pass (ENF-4(c)), raw `OrderSend` on `MqlTradeRequest` with no standard library (ENF-5(a)); filling is per symbol from `SYMBOL_FILLING_MODE`, FOK else IOC else RETURN, deviation a compile-time constant (ENF-6(a)); the ordering is pendings first, then positions by floating loss most negative first, ties by ticket (ENF-7(a)), no close-by pairing (ENF-8(a)); the backoff is class based, doubling 1, 2, 4, 8, 16, 32 capped at 60 for the retry class and no attempts for the hold class until its condition changes (ENF-9(c)), measured on a timer pass counter and never a clock (ENF-10(a)); the hard stop is H=10 attempts per ticket for the retry class, at which the ticket is HELD with one ALERT naming it, the `sweep held` journal line at the cadence, the CANNOT_FLATTEN sub-condition on the banner, and a re-arm for one attempt when the retcode class changes or every 60 passes (ENF-11(a)); a `DONE_PARTIAL` resets the ticket's attempt counter and the remainder is a fresh ticket for the schedule (ENF-12(a)); before every send the symbol's own trade session table and `SYMBOL_TRADE_MODE` are pre-checked and the retcode is classified as well, the name from the pre-check and the truth from the retcode (ENF-13(c)); `SYMBOL_TRADE_MODE_CLOSEONLY` is sendable, enumerated and logged distinctly (ENF-15(a)); flat is `PositionsTotal() == 0` and zero pending-type orders on two consecutive passes that sent nothing, and `sweep complete` fires once per flatten episode (ENF-16(b)); every guardian request carries the compile-time magic and the comment `AG sweep` (ENF-19(a)); on a boot-derived lock the sweep starts at the SYNCING exit, on the first LOCKED dispatch, LOCKED being the only state whose dispatch sweeps (ENF-21(a)); expiry is unchanged, `TimeCurrent >= locked_until` regardless of the book, the sweep stops with the state and a held position rides into the new day under question FIVE (ENF-22(a)). Nothing about the sweep is persisted.
- Trade-disallowed states (Q3, FINAL), each enumerated and logged distinctly: TERMINAL_TRADE_ALLOWED false, MQL_TRADE_ALLOWED false, ACCOUNT_TRADE_ALLOWED false, ACCOUNT_TRADE_EXPERT false, investor-password login, symbol session closed or close-only. All are kill-equivalent: detect-only, continuous loud alerting (journal + Alert + chart banner), immediate sweep the moment trading is restored. The formal promise: flattened within seconds of the platform accepting a close for that symbol. Read per owner ruling ENF-14(b) of 2026-09-16: "continuous loud alerting" means the `sweep blocked` journal line at the cadence and the CANNOT_TRADE sub-condition on the chart banner; the Alert popup fires once at entry to the blocked state and once at exit, not at the cadence. Close-only is read per ENF-15(a): the state is named and the close is sent, since the platform accepts it.
- Disconnection: mobile trades run unswept while the terminal is offline. Kill-equivalent, accepted. On reconnect (ENF-20(a), 2026-09-16, read against the machine as built, where LOCKED has no SYNCING re-entry): the sweep sends nothing from a disconnected pass, reads `TERMINAL_CONNECTED` fresh on every pass, and resumes on the first connected tick with the coverage gap logged as `sweep resumed|gap=<disconnect stamp>..<reconnect stamp>`.
...
- Sweep: one line per close or delete attempt with retcode; failure reasons distinct; trade-disallowed states enumerated by name. The line names as built (owner rulings of 2026-09-16, plan 3.5), every one a pure formatter in SweepPolicy.mqh proven by the vectors: `sweep pass|positions=<n>|pendings=<m>|held=<h>|sent=<s>` once per pass that sent anything and once when the book first reads flat; `sweep delete|order=<t>|symbol=<s>|type=<pending type>|retcode=<r>|class=<retry|hold|refuse|done|placed>|attempt=<k>` one per attempt; `sweep close|position=<t>|symbol=<s>|type=<buy|sell>|volume=<v>|floating=<f>|retcode=<r>|class=<...>|attempt=<k>|filled=<v'>` one per attempt; `sweep held|position=<t>|symbol=<s>|reason=<...>|next_open=<t>|since=<t>` (or `order=<t>`) at hold entry and at the cadence per held ticket; `sweep blocked|state=<Q3 name>` at entry to an account-wide block and at the cadence; `sweep complete|positions=0|pendings=0|attempts=<n>|elapsed=<passes>` once per flatten episode; `sweep resumed|gap=<t1>..<t2>` on reconnect; `sweep accelerated|transaction=DEAL_ADD|deal=<n>` per accelerator firing.
- Alert popups mandatory for: breach, lock, unlock, state-write failure, cannot-trade-while-locked, SAFE_HALT. The lock and boot ALERT read `DAILY_BREACH: account LOCKED until <t> (pnl=<p> limit=<l>). Flattening <n> positions and deleting <m> pending orders.` (ENF-18(a); the boot ALERT keeps its own head naming the derivation reason). Cannot-trade-while-locked cadence per ENF-14(b): one popup at entry to the blocked state and one at exit; one popup at the first hold of a ticket and one when a held ticket is finally flattened (ENF-11(a)).
- Chart banner always shows: current state, lock reason, locked_until, daily PnL vs limit. Under LOCKED the banner carries the CANNOT_TRADE sub-condition while an account-wide state blocks and the CANNOT_FLATTEN sub-condition while any ticket is held (section 2, ENF-11(a)).
...
Rows mapped to the acceptance rows of docs/PLAN_ENFORCEMENT_SWEEP_2026-09-16.md section 3.10 under owner ruling ENF-29(a) of 2026-09-16: two live locks, a daytime lock serving ENF-L1, L2, L3, L5 and L6 in that order and a night or weekend lock serving ENF-L4 and L7; every failure of a live row is reported to the owner immediately as a numbered defect.
- Breach flattens EA, manual, and mobile positions, and deletes pendings. ENF-L1.
- New mobile position while locked closed within seconds; both timer and OnTradeTransaction paths exercised. ENF-L2.
- AutoTrading toggled off while locked: continuous alert (per ENF-14(b), the journal line and the banner, one popup at entry and one at exit), no crash, sweep resumes on re-enable. ENF-L3.
- Closed-session symbol: bounded retries, distinct logs, no order storm. ENF-L4, with the expiry of that lock carrying the held position into the new day as ENF-L7.
- Boot with positions open (ENF-L5) and disconnect while locked (ENF-L6), added by the ruling.
- Partial close and hedging-account matrix. The hedging half is every close on this account, each bound to its position ticket. A partial close is NOT producible on demand: under FOK, the filling mode on record, a close fills whole or not at all, so the `DONE_PARTIAL` path is proven by the vectors (ENF-0) and measured live only if an IOC symbol ever fills in pieces.
